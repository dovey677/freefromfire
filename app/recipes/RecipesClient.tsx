...
export default function RecipesClient({ initialRecipes }: { initialRecipes: any[] }) {
              ...
              <Link key={recipe._id} href={`/recipe/${recipe.slug}`} style={{ textDecoration: 'none' }}>
                <div style={{ backgroundColor: '#2a2020', border: '1px solid #3a2a2a', borderRadius: '12px', overflow: 'hidden', transition: 'border-color 0.2s' }}
                  ...
                  <div style={{ padding: '0rem 1.25rem 1.25rem 1.25rem' }}>
                    ...
                    {recipe.summary && (
                      <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.8rem', color: '#7A8F6A', lineHeight: 1.5, margin: '0.5rem 0 0.5rem 0', display: '-webkit-box', WebkitLineClamp: 5, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                        {recipe.summary}
                      </p>
                    )}
